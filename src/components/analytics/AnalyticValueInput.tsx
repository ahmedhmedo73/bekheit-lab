...
export function AnalyticValueInput({ child, prefix, disabled, onChange }: {
  ...
  return <div className={`result-value-row ${hasAnalyticResultValue(child) ? 'is-complete' : ''}`}>
    ...
    </> : <Input id={id} label={`${child.name}${child.resultType === 'differential' ? ' — Relative Count' : ''}`} value={child.result} disabled={disabled}
